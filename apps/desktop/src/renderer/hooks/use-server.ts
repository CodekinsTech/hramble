...
async function buildBrainRecallPart(
	...
	try {
		...
		// DISPLAY mirror — same items, mapped into typed chips for the docked-Brain
		// header. Never affects what the agent receives.
...
async function buildEpisodeRecallPart(
	...
	try {
		...
		// DISPLAY mirror — past-task matches become "episode" chips in the header.
		const contributions: BrainContribution[] = matches.map((m) => {
